...
data_dir <- file.path("data", "SRP155976")

# Define the file path to the data directory
results_dir <- file.path("data", "SRP155976","results")

# Declare the file path to the gene expression matrix file inside directory saved as `data_dir`
data_file <- file.path(data_dir, "SRP155976.tsv")

# Declare the file path to the metadata file inside the directory saved as `data_dir`
metadata_file <- file.path(data_dir, "metadata_SRP155976.tsv")
...
head(metadata$refinebio_disease)
table(metadata$refinebio_disease)

# ==== Setup metadata ====
# create a new column with cancer/control status of each sample
metadata <- metadata %>%
  # Let's get the status from this variable
  dplyr::mutate(status = dplyr::case_when(
    stringr::str_detect(refinebio_disease, "cd") ~ "disease",
    stringr::str_detect(refinebio_disease, "uc") ~ "disease",
    stringr::str_detect(refinebio_disease, "control") ~ "healthy"
  ))

dplyr::select(metadata, refinebio_disease, status)
...
write.table(DE_gene_ids, file = "differential_expression_gene_ids_SRP155976.txt", col.names = FALSE, quote = FALSE, row.names = FALSE)
...
write.table(top_20_genes, file = "differential_expression_top_20_gene_ids_SRP155976.txt", col.names = FALSE, quote = FALSE, row.names = FALSE)
